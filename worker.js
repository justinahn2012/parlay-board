...
export class Board {
  ...
  async fetch(req) {
    ...
    /* Lines are fetched outside the object and applied here, as scores are, so
       the network call never holds the lock. A line only lands on a week that
       is still to be played, so a cron run can never rewrite a settled price. */
    if (path === '/apply-lines' && req.method === 'POST') {
      ...
      let n = 0;
      Object.keys(lines).forEach(wk => {
        const w = (obj.weeks || {})[wk];
        if (!w) return;
        if (weekPastDue(w) || w.locked) return;          // played or locked: leave it
        w.lines = w.lines || {};
        Object.keys(lines[wk]).forEach(id => {
          if (!(w.games || []).some(g => g.id === id)) return;   // unknown game
          const prev = w.lines[id] || {};
          if (prev.src === 'manual') return;              // never overwrite a hand-set line
          const next = lines[wk][id];
          /* Only write when a number actually moved. The timestamp always
             differs, so comparing whole objects would rewrite the board on
             every run and force every open client to re-pull for nothing. */
          const same = ['as','hs','ao','ho','am','hm','ssrc','msrc']
            .every(k => next[k] === undefined || prev[k] === next[k]);
          if (same) return;
          w.lines[id] = Object.assign({}, prev, next);
          n++;
        });
      });
...
/* ---------------------------------------------------------------------------
   Lines, refreshed on a schedule.

   The board used to price weeks only when somebody opened it, which meant the
   numbers were as stale as the last visit. This runs on the cron with nobody
   on the page. It is a deliberate second implementation of the matching the
   page does -- the page is inlined here as a string and cannot be called -- so
   both are covered by their own tests and the shapes are kept in step.
   --------------------------------------------------------------------------- */
const PM_BASE = 'https://gamma-api.polymarket.com/';
const AB_ALIAS = { WSH: 'WAS', LA: 'LAR', LARM: 'LAR', STL: 'LAR', SD: 'LAC', OAK: 'LV',
  ARZ: 'ARI', BLT: 'BAL', CLV: 'CLE', HST: 'HOU', JAC: 'JAX', TAM: 'TB', NWE: 'NE',
  NOR: 'NO', SFO: 'SF', KAN: 'KC', GNB: 'GB', NYA: 'NYJ' };
const CLUBS = { NE:'Patriots', SEA:'Seahawks', SF:'49ers', LAR:'Rams', CHI:'Bears',
  CAR:'Panthers', TB:'Buccaneers', CIN:'Bengals', BAL:'Ravens', IND:'Colts', BUF:'Bills',
  HOU:'Texans', NO:'Saints', DET:'Lions', NYJ:'Jets', TEN:'Titans', ATL:'Falcons',
  PIT:'Steelers', CLE:'Browns', JAX:'Jaguars', ARI:'Cardinals', LAC:'Chargers',
  GB:'Packers', MIN:'Vikings', MIA:'Dolphins', LV:'Raiders', WAS:'Commanders',
  PHI:'Eagles', DAL:'Cowboys', NYG:'Giants', DEN:'Broncos', KC:'Chiefs' };
const COMPANION = /-(player-props|halftime-result|second-half-result|exact-score|first-to-score|more-markets|props|totals)$/;

const toAmerican = p => p >= 0.5 ? -Math.round(100 * p / (1 - p)) : Math.round(100 * (1 - p) / p);
const normAb = v => String(v || '').toUpperCase().replace(/[^A-Z]/g, '');
const jparse = v => { try { return typeof v === 'string' ? JSON.parse(v) : v } catch (e) { return null } };
const mktType = m => String((m.sportsMarketType) || (m.sports && m.sports.sportsMarketType) || '').toLowerCase();

async function pmJson(path, qs) {
  const r = await proxy(PM_BASE + path + (qs ? '?' + qs : ''));
  if (!r.ok) throw new Error(path + ' ' + r.status);
  return r.json();
}
async function pmClubMap() {
  const map = {};
  try {
    const j = await pmJson('teams', 'league=nfl&limit=64');
    const list = Array.isArray(j) ? j : (j && j.teams) || [];
    list.forEach(t => {
      const nm = String(t.name || '').toLowerCase(), ab = normAb(t.abbreviation);
      if (!ab || !nm) return;
      Object.keys(CLUBS).forEach(code => { if (nm.indexOf(CLUBS[code].toLowerCase()) > -1) map[ab] = code });
    });
  } catch (e) {}
  return map;
}
async function pmList(filter) {
  const out = [];
  let cursor = null;
  try {
    for (let i = 0; i < 10; i++) {
      const j = await pmJson('events/keyset',
        (filter || '') + 'closed=false&limit=100' + (cursor ? '&after_cursor=' + encodeURIComponent(cursor) : ''));
      const page = (j && j.events) || [];
      if (!page.length) break;
      page.forEach(x => out.push(x));
      cursor = j && (j.next_cursor || j.nextCursor);
      if (!cursor) break;
    }
  ...
  for (let off = 0; off < 600; off += 100) {
    const page = await pmJson('events', (filter || '') + 'closed=false&active=true&limit=100&offset=' + off);
    if (!Array.isArray(page) || !page.length) break;
    page.forEach(x => out.push(x));
    if (page.length < 100) break;
  }
...
function sideOf(ev, g, map) {
  const teams = ev.teams || (ev.sports && ev.sports.teams) || [];
  if (teams.length === 2) {
    const code = t => {
      const c = normAb(t.abbreviation);
      if (map && map[c]) return map[c];
      if (AB_ALIAS[c]) return AB_ALIAS[c];
      const nm = String(t.name || '').toLowerCase();
      if (nm) for (const k in CLUBS) if (nm.indexOf(CLUBS[k].toLowerCase()) > -1) return k;
      return c;
    };
    const codes = teams.map(code);
    return codes.indexOf(g.a[0]) > -1 && codes.indexOf(g.h[0]) > -1;
  }
  const hay = ((ev.title || '') + ' ' + (ev.slug || '')).toLowerCase();
  const an = String((g.a[1] || '')).toLowerCase(), hn = String((g.h[1] || '')).toLowerCase();
  return !!an && !!hn && hay.indexOf(an) > -1 && hay.indexOf(hn) > -1;
}
function moneylineOf(ev, g) {
  const an = String(g.a[1] || '').toLowerCase(), hn = String(g.h[1] || '').toLowerCase();
  const pick = m => {
    const outs = jparse(m.outcomes), pr = jparse(m.outcomePrices);
    if (!Array.isArray(outs) || !Array.isArray(pr) || outs.length !== 2) return null;
    const lo = outs.map(x => String(x).toLowerCase());
    ...
    const pa = parseFloat(pr[ai]), ph = parseFloat(pr[hi]);
    if (!(pa > 0 && pa < 1 && ph > 0 && ph < 1)) return null;
    return { am: toAmerican(pa), hm: toAmerican(ph) };
  };
  for (const m of (ev.markets || [])) if (mktType(m) === 'moneyline') { const v = pick(m); if (v) return v }
  for (const m of (ev.markets || [])) { const v = pick(m); if (v) return v }
  return null;
}
/* Polymarket lists a slider of alternate handicaps as separate markets. Keep
   the one priced nearest an even chance -- that is the primary line -- and
   reject anything that is not a plausible NFL handicap. */
function spreadOf(ev, g) {
  const an = String(g.a[1] || '').toLowerCase(), hn = String(g.h[1] || '').toLowerCase();
  const num = v => { const n = parseFloat(v); return isFinite(n) ? Math.abs(n) : null };
  const plausible = n => n !== null && n >= 0.5 && n <= 30 && Math.abs(n * 2 - Math.round(n * 2)) < 1e-9;
  const cands = [];
  (ev.markets || []).forEach(m => {
    ...
    if (yi < 0) return;
    const py = parseFloat(pr[yi]), pn = parseFloat(pr[yi === 0 ? 1 : 0]);
    if (!(py > 0 && py < 1) || !(pn > 0 && pn < 1)) return;
    const txt = String(m.question || m.title || m.slug || '').toLowerCase();
    let line = num(m.line) || num(m.spread) || num(m.sports && (m.sports.line || m.sports.spread));
    if (!plausible(line)) {
      line = null;
      const pats = [/by\s+(?:more\s+than\s+)?(\d+(?:\.\d+)?)/, /[+-](\d+(?:\.\d+)?)\s*$/,
        /\(([+-]?\d+(?:\.\d+)?)\)/, /\s([+-]\d+(?:\.\d+)?)\b/];
      for (const re of pats) { const hit = txt.match(re); if (hit) { const v = num(hit[1]); if (plausible(v)) { line = v; break } } }
    }
    if (!plausible(line)) return;
    const ai = txt.indexOf(an), hi = txt.indexOf(hn);
    let favHome = null;
    const beat = txt.match(/([a-z0-9' .-]+?)\s+beat\s+(?:the\s+)?([a-z0-9' .-]+)/);
    if (beat) { if (beat[1].indexOf(hn) > -1) favHome = true; else if (beat[1].indexOf(an) > -1) favHome = false }
    if (favHome === null && ai > -1 && hi > -1) favHome = hi < ai;
    else if (favHome === null && hi > -1) favHome = true;
    else if (favHome === null && ai > -1) favHome = false;
    if (favHome === null) return;
    /* De-vigged, using both posted prices: Yes alone ties 51/50 with 49/52
       even though they are 50.5% and 48.5% once the hold is removed. */
    const fair = py / (py + pn);
    cands.push({ line, py, pn, favHome, off: Math.abs(fair - 0.5), hold: py + pn - 1 });
  });
  if (!cands.length) return null;
  cands.sort((a, b) => a.off - b.off || a.hold - b.hold || a.line - b.line);
  const best = cands[0];
  return best.favHome
    ? { hs: -best.line, as: best.line, ho: toAmerican(best.py), ao: toAmerican(best.pn) }
    : { as: -best.line, hs: best.line, ao: toAmerican(best.py), ho: toAmerican(best.pn) };
}
function matchGames(ev, gs, map) {
  const got = {};
  const gid = e => e.gameId || (e.sports && e.sports.gameId) || null;
  const byGame = {};
  ev.forEach(e => { const id = gid(e); if (id != null) (byGame[id] = byGame[id] || []).push(e) });
  gs.forEach(g => {
    let mine = ev.filter(x => sideOf(x, g, map));
    [...new Set(mine.map(gid).filter(x => x != null))].forEach(id => {
      (byGame[id] || []).forEach(e => { if (mine.indexOf(e) < 0) mine.push(e) });
    });
    mine.sort((a, b) => (COMPANION.test(String(a.slug || '')) ? 1 : 0) - (COMPANION.test(String(b.slug || '')) ? 1 : 0));
    let ml = null, sp = null;
    for (const e of mine) {
      if (!ml) ml = moneylineOf(e, g);
      if (!sp) sp = spreadOf(e, g);
      if (ml && sp) break;
    }
    if (!ml && !sp) return;
    got[g.id] = Object.assign({}, ml || {}, sp || {});
  });
  return got;
}
/* ESPN supplies the spread where Polymarket has none. One request per game
   day; a range query returns nothing and is never used. */
async function espnOdds(gs) {
  const days = [...new Set(gs.map(g => g.d || (g.ko ? String(g.ko).slice(0, 10) : null)).filter(Boolean))].sort();
  const ev = [];
  for (const d of days.slice(0, 12)) {
    try {
      const r = await proxy('https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?limit=100&dates=' + d.replace(/-/g, ''));
      if (!r.ok) continue;
      const j = await r.json();
      (j.events || []).forEach(x => ev.push(x));
    } catch (e) {}
  ...
  gs.forEach(g => {
    const e = ev.filter(x => {
      const c = (x.competitions || [])[0]; if (!c) return false;
      const ab = t => { const v = normAb((t.team || {}).abbreviation); return AB_ALIAS[v] || v };
      return (c.competitors || []).some(t => ab(t) === g.a[0]) && (c.competitors || []).some(t => ab(t) === g.h[0]);
    })[0];
    const c = e && (e.competitions || [])[0], o = c && (c.odds || [])[0];
    if (!o || !o.details) return;
    const t = String(o.details).trim();
    const m = t.match(/^([A-Z]{2,4})\s*([+-]?\d+(?:\.\d+)?)$/i);
    if (!m) return;
    const ab = m[1].toUpperCase(), n = parseFloat(m[2]);
    if (!isFinite(n)) return;
    const jz = v => isFinite(Number(v)) ? Math.round(Number(v)) : -110;
    const out = ab === g.h[0] ? { hs: n, as: -n } : ab === g.a[0] ? { as: n, hs: -n } : null;
    if (!out) return;
    out.ao = jz(o.awayTeamOdds && o.awayTeamOdds.spreadOdds);
    out.ho = jz(o.homeTeamOdds && o.homeTeamOdds.spreadOdds);
    got[g.id] = out;
...
/* The whole job: price the weeks that are still to be played. The network work
   happens here; the object is only touched for the write. */
async function refreshLines(env) {
  const s = stub(env);
  const cur = await (await s.fetch('https://do/state')).json();
  if (!cur.value) return { ok: false, reason: 'no board' };
  let state; try { state = JSON.parse(cur.value); } catch (e) { return { ok: false, reason: 'unparseable' }; }

  const weeks = Object.keys(state.weeks || {}).map(Number).filter(n => {
    const w = state.weeks[n];
    if (!w || !(w.games || []).length) return false;
    return !weekPastDue(w) && !w.locked;
  }).sort((a, b) => a - b).slice(0, 5);
  if (!weeks.length) return { ok: true, changed: 0, reason: 'nothing upcoming' };

  const map = await pmClubMap();
  let ev = [];
  try {
    const tj = await pmJson('tags/slug/nfl');
    const tag = tj && (tj.id || (Array.isArray(tj) && tj[0] && tj[0].id));
    if (tag) ev = await pmList('tag_id=' + tag + '&');
  } catch (e) {}
  if (!ev.length) { try { ev = await pmList('') } catch (e) {} }

  const lines = {}, priced = {};
  for (const n of weeks) {
    const gs = state.weeks[n].games || [];
    const pm = matchGames(ev, gs, map);
    let es = {};
    try { es = await espnOdds(gs) } catch (e) {}
    const out = {};
    gs.forEach(g => {
      const L = {}, p = pm[g.id], e = es[g.id];
      let touched = false;
      if (p && p.as !== undefined) { L.as = p.as; L.hs = p.hs; L.ao = p.ao; L.ho = p.ho; L.ssrc = 'polymarket'; touched = true }
      else if (e && e.as !== undefined) { L.as = e.as; L.hs = e.hs; L.ao = e.ao; L.ho = e.ho; L.ssrc = 'espn'; touched = true }
      if (p && p.am !== undefined) { L.am = p.am; L.hm = p.hm; L.msrc = 'polymarket'; touched = true }
      if (!touched) return;
      L.src = 'live'; L.at = Date.now();
      out[g.id] = L;
    });
    if (Object.keys(out).length) { lines[n] = out; priced[n] = Object.keys(out).length }
  }
  if (!Object.keys(lines).length) return { ok: true, changed: 0, weeks: weeks, reason: 'no prices found' };

  const r = await s.fetch('https://do/apply-lines', {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ lines })
  });
  const j = await r.json();
  return Object.assign({ weeks: weeks, priced: priced }, j);
}
...
export default {
  async fetch(req, env) {
    ...
    /* Gamma is a read-only public API. `path` selects which documented endpoint
       to hit, from a fixed allowlist -- the board needs /tags/slug/nfl to
       resolve the numeric tag id and /events to list by it, and pinning the
       set keeps this from becoming an open proxy. */
    if (p === '/api/polymarket') {
      const PM_PATHS = {
        events: 'events',                 // legacy offset listing, kept as a fallback
        keyset: 'events/keyset',          // the documented cursor listing
        tag: 'tags/slug/nfl',
        teams: 'teams',                   // authoritative club abbreviations
        series: 'series',
        sports: 'sports',
        types: 'sports/market-types'
      };
      const want = url.searchParams.get('path') || 'events';
      const target = PM_PATHS[want];
      if (!target) return J({ error: 'bad path' }, 400);
      const qs = new URLSearchParams(url.searchParams);
      qs.delete('path');
      const q = qs.toString();
      try {
        return await proxy('https://gamma-api.polymarket.com/' + target + (q ? '?' + q : ''));
    ...
    if (p === '/api/version') return J({
      build: '2026-09-14', weeks: 22, slate: 'auto', lines: 'polymarket+espn',
    ...
    if (p === '/api/diag') {
      ...
      /* Resolve the NFL tag, then list events by it, and report how many of
         those actually carry a moneyline market. This is the check that would
         have shown the integration matching nothing. */
      out.polymarket = {};
      try {
        const tr = await proxy('https://gamma-api.polymarket.com/tags/slug/nfl');
        const tj = await tr.json();
        const tag = tj && (tj.id || (Array.isArray(tj) && tj[0] && tj[0].id));
        out.polymarket.tag = { status: tr.status, id: tag || null };
        if (tag) {
          const er = await proxy('https://gamma-api.polymarket.com/events?tag_id=' + tag
            + '&closed=false&active=true&limit=100');
          const ev = await er.json();
          const list = Array.isArray(ev) ? ev : [];
          const ml = list.filter(e => (e.markets || []).some(m =>
            String(m.sportsMarketType || '').toLowerCase() === 'moneyline')).length;
          const withTeams = list.filter(e => (e.teams || []).length === 2).length;
          out.polymarket.events = { status: er.status, count: list.length,
            withMoneyline: ml, withTeams: withTeams,
            sample: list.slice(0, 3).map(e => ({ title: e.title, slug: e.slug,
              teams: (e.teams || []).map(x => x.abbreviation + ':' + (x.ordering || '?')),
              types: (e.markets || []).map(m => m.sportsMarketType || '?'),
              /* The exact wording and fields of a spread market, which I have
                 never seen. Without this the handicap has to be guessed. */
              spreads: (e.markets || [])
                .filter(m => String(m.sportsMarketType || '').toLowerCase() === 'spreads')
                .slice(0, 3)
                .map(m => ({ question: m.question, line: m.line, spread: m.spread,
                  outcomes: m.outcomes, prices: m.outcomePrices,
                  keys: Object.keys(m).filter(k => /line|spread|handicap|point/i.test(k)) })) })) };
        }
      } catch (e) { out.polymarket.error = String(e); }

      /* Actually fetch crests rather than assume the paths are right. The three
         checked are the ones whose slug is not simply the abbreviation. */
      out.logos = {};
      for (const [name, path] of [['washington', '500/wsh'], ['rams', '500/lar'], ['panthers', '500-dark/car']]) {
        const url = 'https://a.espncdn.com/i/teamlogos/nfl/' + path + '.png';
        const t0 = Date.now();
        try {
          const r = await fetch(url, { method: 'GET', headers: { 'user-agent': AGENTS[goodAgent] } });
          const buf = r.ok ? await r.arrayBuffer() : null;
          out.logos[name] = { status: r.status, type: r.headers.get('content-type'),
            bytes: buf ? buf.byteLength : 0, ms: Date.now() - t0, url };
        } catch (e) { out.logos[name] = { error: String(e), url }; }
      }
      out.logosOk = Object.values(out.logos).every(x => x.status === 200 && x.bytes > 1000);
      return J(out);
    }
  ...
  async scheduled(event, env, ctx) {
    ...
    ctx.waitUntil(refreshLines(env).catch(e => console.log('line refresh failed', e)));
  }
};

export const _test = { authorize, pending, sha, sameHash, syncScores, refreshLines,
  matchGames, spreadOf, moneylineOf, sideOf, toAmerican };
