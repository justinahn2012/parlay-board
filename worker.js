...
function spreadOf(ev, g) {
  ...
  (ev.markets || []).forEach(m => {
    ...
    if (favHome === null) return;
    cands.push({ line, py, favHome, off: Math.abs(py - 0.5) });
  });
  if (!cands.length) return null;
  cands.sort((a, b) => a.off - b.off || a.line - b.line);
  const best = cands[0];
  return best.favHome
    ? { hs: -best.line, as: best.line, ho: toAmerican(best.py), ao: toAmerican(1 - best.py) }
    : { as: -best.line, hs: best.line, ao: toAmerican(best.py), ho: toAmerican(1 - best.py) };
}
