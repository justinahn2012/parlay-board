...
function authorize(prev, next, user) {
  ...
  for (const wk of weeks) {
    const A = (prev.weeks || {})[wk], B = (next.weeks || {})[wk];
    if (!A && B) return 'Only a moderator can add a week';
    if (A && !B) return 'Only a moderator can delete a week';
    if (S(A) === S(B)) continue;

    if (S(A.games) !== S(B.games)) return 'Only a moderator can change the slate';
...
async function fetchFinals(dates) {
  ...
  const r = await fetch(
    ...
    { headers: { 'user-agent': 'parlay-board' } });
...
async function proxy(target) {
  const r = await fetch(target, { headers: { 'user-agent': 'parlay-board' } });
...
export default {
  async fetch(req, env) {
    ...
    if (p === '/api/version') return J({ build: '2026-09-07d', weeks: 22, lines: 'espn' });
